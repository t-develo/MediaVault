...
- **avi / wmv / mkv / flv も再生可能（シーク対応）** — ブラウザ非対応の形式は `ffmpeg` で H.264/AAC の MP4 へ**バックグラウンド事前変換してディスクにキャッシュ**し、以後は HTTP Range で配信します。**シーク可能・再変換なし**でラズパイの負荷を抑えます（`ffmpeg` が必要）。未変換のファイルを開いた場合は変換の進捗を表示して完了を待ちます。変換キャッシュは上限サイズを超えると古いものから自動削除されます。
...
- Go 1.25 以上（ビルド時のみ）
- `ffmpeg`（`ffprobe` 同梱）（**動画サムネイル生成と avi/wmv 等の変換再生に使用。無くても動作します** — その場合、動画サムネはアイコン表示になり、avi/wmv 等のブラウザ非対応形式は再生できません）
...
internal/convert   avi/wmv等の事前変換（ffmpeg）＋ディスクキャッシュ・LRU
